...
# heuristic = MyModule.train_heuristic_on_data(heuristic, training_samples[1:1000])
optimizer = Adam()
...
#             serialize("models/training_sample_inf_grad.bin", sample)
#             @assert 0 == 1
#         end
...
#     end
# end
# serialize("models/trainied_heuristic1000sorted_samples3.bin", heuristic)
...
function test_different_searches(heuristic, data, max_steps=1000,max_depth=50)
    ...
    CSV.write("profile_results_trained_heuristic1.csv", df)
    # df = map(data) do ex
    # # @benchmark begin
    #     exp_cache = LRU{MyModule.ExprWithHash, Vector}(maxsize=100_000)
    #     cache = LRU{MyModule.ExprWithHash, Vector}(maxsize=1_000_000)
...
function updated_test_different_searches()
...
function plot_hist_comparison()
    ...
    df3 = CSV.read("profile_results_trained_heuristic.csv", DataFrame)
