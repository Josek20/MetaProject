...
intern!(ex) = get!(nc[], ex)
intern!(ex::Nothing) = nothing

expr(ex::NodeID) = expr(nc[], ex)

Base.:(>)(x::NodeID, y::Number) = nc[][x.id].head isa Number ? nc[][x.id].v > y : error("comparing non number to number") 
Base.:(<)(x::NodeID, y::Number) = nc[][x.id].head isa Number ? nc[][x.id].v < y : error("comparing non number to number") 
Base.:(!=)(x::NodeID, y::Number) = nc[][x.id].head isa Number ? nc[][x.id].v != y : true
Base.:(==)(x::NodeID, y::Number) = nc[][x.id].head isa Number ? nc[][x.id].v == y : false