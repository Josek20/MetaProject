...
@everywhere function ex2mill(ex::Expr, symbols_to_index, all_symbols, variable_names::Dict)
    if ex.head == :call
...
@everywhere function ex2mill(ex::Symbol, symbols_to_index, all_symbols, variable_names)
    n = length(symbols_to_index) + 2
...
@everywhere my_sigmoid(x, k = 0.01, m = 0) = 1 / (1 + exp(-k * (x - m)))


@everywhere function ex2mill(ex::Number, symbols_to_index, all_symbols, variable_names)
    n = length(symbols_to_index) + 2
...
@everywhere function args2mill(args::Vector, symbols_to_index, all_symbols, variable_names)
...
#      
# end
@everywhere function (m::ExprModel)(ds::ProductNode)
    m.heuristic(embed(m, ds))
end

@everywhere function embed(m::ExprModel, ds::ProductNode) 
    # @show ds.data
...
@everywhere function embed(m::ExprModel, ds::BagNode)
    tmp = embed(m, ds.data)
    m.aggregation(tmp, ds.bags)
end

@everywhere embed(m::ExprModel, ds::Missing) = missing
